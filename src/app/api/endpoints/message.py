...
from src.app.schemas.message import MessageDB, MessageCreate
from src.app.schemas.ticket import TicketCreate
...
async def create_message(
    ...
    if request.telegram_id:
        ticket = await check_previous_ticket_closed(request.telegram_id, session)
        if not ticket:
            ticket = await ticket_crud.create(request, session)
        ticket_id = ticket.id
    my_message = dict(text=request.text, ticket_id=ticket_id)
    message =  await message_crud.create(my_message, user=user, session=session)
    return message
